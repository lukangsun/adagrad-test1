...
SCAdagrad.__doc__ = r"""Implements Adagrad algorithm.
    ...
    *,
    ...
    for (param, grad, state_sum, step_t) in zip(params, grads, state_sums, state_steps):
        ...
        if grad.is_sparse:
            ...
            std_tmp = std._values().mul_(-1e-1).clip_(min=-1)
            std_tmp = std_tmp.exp_()
            std_tmp = std_tmp.mul_(1e-1)
            std_values = std._values().add_(std_tmp)
            param.add_(
            ...
            if differentiable:
                std_tmp = state_sum.mul(-1e-2)#.clamp(min=-5e0)
                std_tmp = std_tmp.exp()
                std_tmp = std_tmp.mul(1e-2)
                std = state_sum.add(std_tmp)
            else:
                std_tmp = state_sum.mul_(-1e-2)#.clamp_(min=-5e0)
                std_tmp = std_tmp.exp_()
                std_tmp = std_tmp.mul_(1e-2)
                std = state_sum.add_(std_tmp)
...
def _multi_tensor_adagrad(
    ...
    *,
    ...
    for device_params, device_grads, device_state_sums, device_state_steps in grouped_tensorlists.values():
        ...
        std = torch._foreach_add(device_state_sums, torch._foreach.exp(torch._foreach_mul(device_state_sums,-0.1)))
