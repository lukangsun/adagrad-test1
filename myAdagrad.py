...
myAdagrad.__doc__ = r"""Implements myAdagrad algorithm.
    ...
    *,
    ...
    for (param, grad, state_sum, step_t) in zip(params, grads, state_sums, state_steps):
        ...
        if grad.is_sparse:
            ...
            std_values_tmp = std.values().add_(1e-15).log_().add_(1.00)
            std_values = std._values().add_(-std_values_tmp).div(std_values().add_(-1.00).pow_(2).add_(1e-15))#(std._values()-1-torch.log(std._values()))/((std._values()-1).pow(2))#std._values().sqrt_().add_(eps)
            param.add_(
            ...
            epsilon=1e-20
            if differentiable:
                std_tmp = state_sum.div(epsilon).log()
                std = state_sum.add(-std_tmp.mul(epsilon)-epsilon).div((state_sum-epsilon).pow(2)+1e-15)#state_sum.sqrt() + eps
            else:
                std_values_tmp = state_sum.div_(epsilon).add_(1e-30).log_().add_(epsilon).mul_(epsilon)
                std_tmp = state_sum.add_(-epsilon)
                std = state_sum.add_(-std_values_tmp).div_(std_tmp.pow_(2).add_(1e-15))#state_sum.sqrt().add_(eps)
            param.addcmul_(grad, std, value=-clr)
            if is_complex:
...
def _multi_tensor_adagrad(
    ...
    *,
    ...
    for device_params, device_grads, device_state_sums, device_state_steps in grouped_tensorlists.values():
        ...
        std_tmp = torch._foreach_log(torch._foreach_add(device_state_sums,1e-15))
        std_tmp1 = torch._foreach_add(device_state_sums,-1.00)
        std = torch._foreach_div(torch._foreach_add(std_tmp1,-std_tmp),torch._foreach_add(torch._foreach_mul(std_tmp1,std_tmp1),1e-15))
        toAdd = torch._foreach_mul(torch._foreach_mul(device_grads, minus_clr), std)
        print('multilinear')
        toAdd = [
